class StatsController < ApplicationController
  ...
  def saveScores
    @id1 = params[:uid1]
    @score1 = params[:wins1]
    @reflections1 = params[:reflections1]
    @id2 = params[:uid2]
    @score2 = params[:wins2]
    @reflections2 = params[:reflections2]

    @user1 = User.find(@id1)
    @user2 = User.find(@id2)

    @user1Stat = @user1.stat
    @user2Stat = @user2.stat

    @user1Stat.wins += @score1
    @user2Stat.wins += @score2

    @user1Stat.reflections += @reflections1
    @user2Stat.reflections += @reflections2
